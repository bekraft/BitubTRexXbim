...
{
    public ModelPlacementStrategy AppliedPlacementStrategy { get; init; }
    public IfcAxisAlignment AppliedAxisAlignment { get; init; }
    public int[] SourceRootPlacementsLabels { get; private set; } = Array.Empty<int>();
    ...
    private IXbimManagedGeometryEngine GeometryEngine => _geometryEngine ??= XbimServices.Current.CreateGeometryManagedEngine();
    ...
    internal void Prepare(CancelableProgressing? cancelableProgress)
    {
        ...
        switch (AppliedPlacementStrategy)
        {
            case ModelPlacementStrategy.NewRootPlacement:
                if (Source.SchemaVersion == global::Xbim.Common.Step21.XbimSchemaVersion.Ifc2X3)
    ...
    private XbimPlacementTree? PlacementTree { get; set; }

    internal float UnitsPerMeterSource { get; set; }

    internal float UnitsPerMeterTarget { get; set; }

    internal IEnumerable<IIfcLocalPlacement> Placements => SourceRootPlacementsLabels.Select(i => Source.Instances[i]).OfType<IIfcLocalPlacement>();
...
public sealed class ModelPlacementTransform : ModelTransformTemplate<ModelPlacementTransformPackage>
{
    ...
    public override string Name => "Model Placement";

    /// <summary>
    /// Default strategy is <see cref="ModelPlacementStrategy.ChangeRootPlacements"/>.
    /// </summary>
    ...
    {
        Log?.LogInformation("({Name}) Applying '{Strategy}' strategy to model.", Name, PlacementStrategy);
        package.Prepare(package.ProgressMonitor);
        Log?.LogInformation("({Name}) Got singleton offset shift of {Shift} [m].", Name, package.SingletonShift * (1.0/package.UnitsPerMeterSource));
